...
Page({
    ...
    countFn(e){
        ...
        if(e.detail.count !='add'){
            ...
            for(let i = 0; i < this.data.goods.length; i++){
                if(e.detail.data._id == this.data.goods[i]._id){
    ...
    onLoad: function (options) {
        ...
        const db_users = wx.cloud.database().collection('users')
        const db_carousel = wx.cloud.database().collection('carousel')
        const db_shops = wx.cloud.database().collection('shops')
                        ...
                        db_users.doc(getApp().globalData._id).get({
                            success(res){
                                ...
                                for(let i = 0; i<that.data.goods.length; i++){
                                    that.data.goods[i].count = 0
                                    for(let j = 0; j<that.data.shop.data.length; j++){
                                        if(that.data.goods[i]._id==that.data.shop.data[j]._id){
                                            that.data.goods[i].count = that.data.shop.data[j].count
                                        }
                                    }
                                }
                                that.setData({
